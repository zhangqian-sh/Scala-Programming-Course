...
object PersistentModel extends Model:
  ...
  def create(task: Task): Id =
    val nextId = Id(loadId().toInt + 1)
    saveTasks(Tasks(loadTasks().toList ++ List(nextId -> task)))
    saveId(nextId)
    nextId

  def read(id: Id): Option[Task] =
    val idStore = mutable.LinkedHashMap.from(loadTasks().toList)
    idStore.get(id)

  def update(id: Id)(f: Task => Task): Option[Task] =
    val idStore = mutable.LinkedHashMap.from(loadTasks().toList)
    val updated_task = idStore.updateWith(id)(opt => opt.map(f))
    saveTasks(Tasks(idStore.toList))
    updated_task

  def delete(id: Id): Boolean =
    val idStore = mutable.LinkedHashMap.from(loadTasks().toList)
    var found = idStore.get(id) match
